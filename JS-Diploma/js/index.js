...
document.addEventListener("DOMContentLoaded", () => {
	...
	getRequest(Request, (response) => {
		...
		//фильтрация залов,  теперь должны оставаться только открытые залы
...
// function showSeances(requestFilms, requestHalls, requestSeances) {const movieList = Array.from(document.querySelectorAll(`.movie`));
// const openHalls = searchItemNode(requestHalls, `hall_open`, 1);
	...
	//   }
	// }  Запуталась , где тут была ошибка , посмотреть позже
